...
landmesses
grep(pattern="New", x=landmesses) #New 찾기
...
sub(pattern="Data", replacement="Business", x=txt) #하나만
gsub(pattern="Data", replacement="Business", x=txt) #모두
...
grep("chas?e", words, value=T) # s가 0번 또는 1번 chae, chase 
grep("chas*e", words, value=T) # chae, chase, chasse, chassse, ...
grep("chas+e", words, value=T) # chase, chasse, ...
